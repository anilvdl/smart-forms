...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const payload = req.body as UpdateCardPayload;
  if (!payload?.paymentMethodId) {
    return res
      .status(400)
      .json({ code: 'BAD_REQUEST', message: 'paymentMethodId is required' });
  }
  ...
  try {
    const url = `${SERVICE_URL}/admin/billing/update-card`;
    const response = await httpClient.post(url, payload, { headers });
    return res.status(response.status).json(response.data);
  } catch (err: any) {
    console.error('POST /billing/update-card error:', err);
    return res
