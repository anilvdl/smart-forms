 ...
 * 
 * Triggers a resend of the magic-link invite with the given ID.
 */
  ...
  if (req.method === 'POST') {
    ...
    try {
      const response = await httpClient.post(
        `${SERVICE_URL}/admin/invites/${encodeURIComponent(id)}/resend`,
        {},
        {
          headers: {
            'x-api-key': API_KEY,
      ...
      console.error('[/api/admin/invites/[id]/resend] error:', err);
